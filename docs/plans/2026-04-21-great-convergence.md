...
**Recommendation: merge the `strategy` repo into `dao`, making dao the single knowledge base for Life Itself.**
...
- **Immediate action (today):** add a strategy/portfolio gateway page to dao.
- **Near-term:** migrate `strategy` repo content into dao. Historical strategy materials go into an archive section.
...
| [`dao`](https://github.com/life-itself/tao) / [dao.lifeitself.org](https://tao.lifeitself.org) | Team playbook: culture, governance, ops, practices, how-we-work | Active |
...
The dao is the natural home for team and collaborator guidance — it already frames "what Life Itself is and how it works" — but it currently has no pointer to the live strategy or portfolio. So it acts as an outdated front door.
...
There is no single front door. Someone new — or returning after absence — must know to look in `strategy` for portfolio, `dao` for how-we-work, `community` for issues, and Drive for project docs. None of these point clearly to each other. The orientation failure is structural, not just a missing page.
...
- **New team members don't know where to look.** Matthew (remote, timezone difference) is the current live example, but the same applies to any new collaborator or returning contributor. The dao exists as a playbook but is disconnected from active portfolio and strategy. The portfolio exists but has no guidance on how to work.
- **Even existing team members lose track of where to put things.** When it is unclear whether a document belongs in `strategy`, `dao`, Drive, or `community`, the path of least resistance is inconsistency — which compounds the discovery problem over time.
...
**Merge `strategy` into `dao`. Make dao the single knowledge base.**

The jobs to be done all require one place. The dao already has the right identity — "the guide to what we stand for and how we operate" — and adding portfolio, initiatives, plans, and meetings makes that actually true rather than aspirationally true.

Historical strategy materials from this repo (the `materials/` folder and older archived docs) go into a dedicated archive section in dao. There is no reason to maintain a separate repo for historical content.
...
### Stage 1: Make dao the front door (today)

Add a strategy and portfolio gateway page to dao linking to this repo and its portfolio map. This is the immediate fix — costs roughly one hour and is already the conclusion from today's team coordination meeting.

### Stage 2: Migrate strategy into dao (near-term)

Move content from `strategy` into dao:
...
- `materials/` — historical strategy archive, into a dedicated archive section in dao
- `people/` — team/collaborator profiles, if not already in dao
...
GitHub issues cannot be moved easily between repos, so any merge of `community` into `dao` would strand the issue history. The knowledge base content there (concepts, organisations, people) also serves a different purpose from the playbook and strategy material. Revisit separately once the strategy/dao merge is done.

### Archive or absorb `comms`

Low activity. Either archive the repo or absorb relevant content (content log, any active issues) into dao or `community` as appropriate.

## Next Steps

1. Rufus and Armelle: add gateway page to dao linking to strategy repo and portfolio map. (Today.)
2. Plan and execute migration of `strategy` content into dao. Agree folder structure in dao first.
